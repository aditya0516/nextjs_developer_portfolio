  ...
  return (
    <Link
      ...
      <FramerImage
        ...
        src={img}
        alt={title}
        className=' z-10 w-96 h-auto hidden rounded-lg absolute md:!hidden'
        style={{ x: x, y: y }}
        initial={{ opacity: 0 }}
...
const FeaturedArticle = ({ img, title, time, summary, link }) => {
  return (
    <li className=' relative col-span-1 w-full p-4 bg-light border border-solid border-dark rounded-2xl dark:bg-dark dark:border-light'>
      ...
      <span className='dark:text-primaryDark text-primary font-semibold'>
        {time}
      </span>
    </li>
  );
...
const Article = ({ img, title, date, link }) => {
  return (
    <motion.li
      ...
      <span className='dark:text-primaryDark text-primary font-semibold pl-4 sm:self-start sm:pl-0 xs:text-sm '>
        {date}
      </span>
    </motion.li>
  );
